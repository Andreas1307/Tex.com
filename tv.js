let mainTv1 = document.querySelector('.main-tv1');
let mainTv2 = document.querySelector('.main-tv2');
...
let productTv1 = [
    {
        id: 1,
        name: 'LED TV Smart VORTEX V32R0212VS, HD, 80cm',
        image: `tv 1.jpg`,
        price: '$159.99',
    ...
    },
    {
        ...
        id: 2,
        name: 'LED TV SCHNEIDER 32-SC410K, HD, 80cm',
        image: 'tv 2 1.jpg',
        price: '$109.99',
...
let productTv2 = [
    {
        id: 4,
        name: 'LED TV VORTEX V32R0212, HD, 81cm',
        image: 'tv 4 1.jpg',
        price: '$89.99',
    ...
    productTv1.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTv(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTv1.appendChild(newDiv)
    });

    productTv2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTv2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTv2.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTv(key){
    if(mainListTv1[key] == null) {
        mainListTv1[key] = productTv1[key];
        mainListTv1[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardTv2(key){
    if(mainListTv2[key] == null) {
        mainListTv2[key] = productTv2[key];
        mainListTv2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    mainListTv1.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity59(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard27.appendChild(newDiv);
        }
    });
    mainListTv2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity60(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard27.appendChild(newDiv);
        }
...
function changeQuantity59(key, quantity) {
    if (quantity == 0) {
        delete mainListTv1[key];
    } else {
        mainListTv1[key].quantity = quantity;
        mainListTv1[key].price = quantity * products[key].price;
    }
    reloadCard()
}

function changeQuantity60(key, quantity) {
    if (quantity == 0) {
        delete mainListTv2[key];
    } else {
        mainListTv2[key].quantity = quantity;
        mainListTv2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
