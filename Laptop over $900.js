let mainLaptopOver = document.querySelector('.main-laptopOver');
...
let listCard19 = document.querySelector('.listCard19');
...
let mainOver = [];
function initApp2() {
    productLaptopsOver.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapOver(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopOver.appendChild(newDiv)
    });
}
initApp2()
function addToCardLapOver(key){
    if(mainOver[key] == null) {
        mainOver[key] = productLaptopsOver[key];
        mainOver[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard19.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainOver.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity23(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard19.appendChild(newDiv);
        }
    });
...
function changeQuantity23(key, quantity) {
    if (quantity == 0) {
        delete mainOver[key];
    } else {
        mainOver[key].quantity = quantity;
        mainOver[key].price = quantity * products[key].price;
    }
    reloadCard()
}
