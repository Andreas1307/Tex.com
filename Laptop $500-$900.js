let mainLaptopBetween = document.querySelector('.main-laptopBetween');
...
let listCard18 = document.querySelector('.listCard18');
...
let mainLapBetween = [];
function initApp2() {
    productLaptopsBetween.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapBetween(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopBetween.appendChild(newDiv)
    });
}
initApp2()
function addToCardLapBetween(key){
    if(mainLapBetween[key] == null) {
        mainLapBetween[key] = productLaptopsBetween[key];
        mainLapBetween[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard18.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainLapBetween.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity19(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard18.appendChild(newDiv);
        }
    });
...
function changeQuantity19(key, quantity) {
    if (quantity == 0) {
        delete mainLapBetween[key];
    } else {
        mainLapBetween[key].quantity = quantity;
        mainLapBetween[key].price = quantity * products[key].price;
    }
    reloadCard()
}
