let mainTvBetween = document.querySelector('.main-tvBetween');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard29 = document.querySelector('.listCard29');
...
let productTvBetween = [
    ...
    },
    {
        ...
        price: '$339.99',
        source: 'samsung qled tv 1.jpg'
    }
]
let mainListTvBetween = [];

function initApp2() {
    productTvBetween.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTvBetween(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTvBetween.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTvBetween(key){
    if(mainListTvBetween[key] == null) {
        mainListTvBetween[key] = productTvBetween[key];
        mainListTvBetween[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard29.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainListTvBetween.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity53(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard29.appendChild(newDiv);
        }
    });
...
function changeQuantity53(key, quantity) {
    if (quantity == 0) {
        delete mainListTvBetween[key];
    } else {
        mainListTvBetween[key].quantity = quantity;
        mainListTvBetween[key].price = quantity * products[key].price;
    }
    reloadCard()
}
