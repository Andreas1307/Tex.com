let mainLaptopUnder = document.querySelector('.main-laptopUnder');
...
let listCard17 = document.querySelector('.listCard17');
...
let mainUnder = [];
function initApp2() {
    productLaptopsUnder.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapUnder(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopUnder.appendChild(newDiv)
    });
}
initApp2()
function addToCardLapUnder(key){
    if(mainUnder[key] == null) {
        mainUnder[key] = productLaptopsUnder[key];
        mainUnder[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard17.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainUnder.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity24(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard17.appendChild(newDiv);
        }
    });
...
function changeQuantity24(key, quantity) {
    if (quantity == 0) {
        delete mainUnder[key];
    } else {
        mainUnder[key].quantity = quantity;
        mainUnder[key].price = quantity * products[key].price;
    }
    reloadCard()
}
