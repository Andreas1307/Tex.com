let mainGamingBetween = document.querySelector('.main-GamingBetween');
...
let listCard24 = document.querySelector('.listCard24');
...
let mainGamBetween = [];
function initApp2() {
    productGamingBetween1.forEach((value, key) => {
        ...
        <div class="slider-box" >
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamBetween1(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingBetween.appendChild(newDiv)
    });
}
initApp2()
function addToCardGamBetween1(key){
    if(mainGamBetween[key] == null) {
        mainGamBetween[key] = productGamingBetween1[key];
        mainGamBetween[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard24.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainGamBetween.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity8(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard24.appendChild(newDiv);
        }
    });
    quantity.innerText = count;
}

function changeQuantity8(key, quantity) {
    if (quantity == 0) {
        delete mainGamBetween[key];
    } else {
        mainGamBetween[key].quantity = quantity;
        mainGamBetween[key].price = quantity * products[key].price;
    }
    reloadCard()
}
