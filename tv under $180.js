let mainTvUnder = document.querySelector('.main-tvUnder');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard28 = document.querySelector('.listCard28');
...
function initApp2() {
    productTvUnder.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTvUnder(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTvUnder.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTvUnder(key){
    if(mainListTvUnder[key] == null) {
        mainListTvUnder[key] = productTvUnder[key];
        mainListTvUnder[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard28.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainListTvUnder.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity58(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard28.appendChild(newDiv);
        }
    });
    quantity.innerText = count;
}

function changeQuantity58(key, quantity) {
    if (quantity == 0) {
        delete mainListTvUnder[key];
    } else {
        mainListTvUnder[key].quantity = quantity;
        mainListTvUnder[key].price = quantity * products[key].price;
    }
    reloadCard()
}
