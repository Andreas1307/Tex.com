let main12gb = document.querySelector('.main-12gb');

let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard7 = document.querySelector('.listCard7');
...
function initApp2() {
    productContent12gb.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCard12gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        main12gb.appendChild(newDiv)
    });
}
initApp2()
function addToCard12gb(key){
    if(mainList12gb[key] == null) {
        mainList12gb[key] = productContent12gb[key];
        mainList12gb[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    let count = 0;
    let totalPrice = 0;
    mainList12gb.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity35(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard7.appendChild(newDiv);
        }
    });
...
function changeQuantity35(key, quantity) {
    if (quantity == 0) {
        delete mainList12gb[key];
    } else {
        mainList12gb[key].quantity = quantity;
        mainList12gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
