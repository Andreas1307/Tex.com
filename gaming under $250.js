let mainGamingUnder1 = document.querySelector('.main-GamingUnder1');
let mainGamingUnder2 = document.querySelector('.main-GamingUnder2');
...
let listCard23 = document.querySelector('.listCard23');
...
let mainGamUnder2 = [];
function initApp2() {
    productGamingUnder1.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamUnder(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingUnder1.appendChild(newDiv)
    });
    productGamingUnder2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamUnder2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingUnder2.appendChild(newDiv)
    });

}
initApp2()
function addToCardGamUnder(key){
    if(mainGamUnder1[key] == null) {
        mainGamUnder1[key] = productGamingUnder1[key];
        mainGamUnder1[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardGamUnder2(key){
    if(mainGamUnder2[key] == null) {
        mainGamUnder2[key] = productGamingUnder2[key];
        mainGamUnder2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard23.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainGamUnder1.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity14(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard23.appendChild(newDiv);
        }
    });

    mainGamUnder2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity15(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard23.appendChild(newDiv);
        }
...
function changeQuantity14(key, quantity) {
    if (quantity == 0) {
        delete mainGamUnder1[key];
    } else {
        mainGamUnder1[key].quantity = quantity;
        mainGamUnder1[key].price = quantity * products[key].price;
    }
    reloadCard()
}

function changeQuantity15(key, quantity) {
    if (quantity == 0) {
        delete mainGamUnder2[key];
    } else {
        mainGamUnder2[key].quantity = quantity;
        mainGamUnder2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
