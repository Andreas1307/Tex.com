let mainTvSmart = document.querySelector('.main-tvSmart');
let mainTvSmart2 = document.querySelector('.main-tvSmart2');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard31 = document.querySelector('.listCard31');
...
let productTvSmart = [
    ...
    {
        id: 3,
        name: 'LED TV Smart VORTEX V40ET1VS, Full HD, 101cm',
        image: 'tv 3 1.jpg',
        price: '$200.00',
    ...
    {
        id: 5,
        name: 'LED TV Smart SAMSUNG 50AU7092, Ultra HD 4K, HDR, 125cm',
        image: 'tv 5 1.jpg',
        price: '$365.50',
...
let mainListTvSmart = [];
let mainListTvSmart2 = [];

function initApp2() {
    productTvSmart.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTvSmart(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTvSmart.appendChild(newDiv)
    });
    productTvSmart2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTvSmart2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTvSmart2.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTvSmart(key){
    if(mainListTvSmart[key] == null) {
        mainListTvSmart[key] = productTvSmart[key];
        mainListTvSmart[key].quantity = 1;
 }    
 reloadCard();
}

function addToCardTvSmart2(key){
    if(mainListTvSmart2[key] == null) {
        mainListTvSmart2[key] = productTvSmart2[key];
        mainListTvSmart2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard31.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainListTvSmart.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity56(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard31.appendChild(newDiv);
        }
    });

    mainListTvSmart2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity57(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard31.appendChild(newDiv);
        }
...
function changeQuantity56(key, quantity) {
    if (quantity == 0) {
        delete mainListTvSmart[key];
    } else {
        mainListTvSmart[key].quantity = quantity;
        mainListTvSmart[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity57(key, quantity) {
    if (quantity == 0) {
        delete mainListTvSmart2[key];
    } else {
        mainListTvSmart2[key].quantity = quantity;
        mainListTvSmart2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
