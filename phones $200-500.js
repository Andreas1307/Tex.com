let mainBetween200 = document.querySelector('.main-between200');
let mainBetween2001 = document.querySelector('.main-between2001');


let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard4 = document.querySelector('.listCard4');
...
function initApp2() {
    productContentBetween200.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardBetween25(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainBetween200.appendChild(newDiv)
    });
    productContentBetween2001.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardBetween252(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainBetween2001.appendChild(newDiv)
    });

}
initApp2()
function addToCardBetween25(key){
    if(mainBetween25[key] == null) {
        mainBetween25[key] = productContentBetween200[key];
        mainBetween25[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardBetween252(key){
    if(mainBetween252[key] == null) {
        mainBetween252[key] = productContentBetween2001[key];
        mainBetween252[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    let count = 0;
    let totalPrice = 0;
    mainBetween25.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity28(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard4.appendChild(newDiv);
        }
    });

    mainBetween252.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity29(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard4.appendChild(newDiv);
        }
...
function changeQuantity28(key, quantity) {
    if (quantity == 0) {
        delete mainBetween25[key];
    } else {
        mainBetween25[key].quantity = quantity;
        mainBetween25[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity29(key, quantity) {
    if (quantity == 0) {
        delete mainBetween252[key];
    } else {
        mainBetween252[key].quantity = quantity;
        mainBetween252[key].price = quantity * products[key].price;
    }
    reloadCard()
}
