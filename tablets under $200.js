let mainTabletsUnder = document.querySelector('.main-tabletUnder1');
...
let listCard11 = document.querySelector('.listCard11');
...
function initApp2() {
    productTabletsUnder.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTabUnder(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTabletsUnder.appendChild(newDiv)
    });
...
initApp2()
function addToCardTabUnder(key){
    if(mainTabUnder[key] == null) {
        mainTabUnder[key] = productTabletsUnder[key];
        mainTabUnder[key].quantity = 1;
 }    
 reloadCard();
}

function reloadCard() {
    listCard11.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainTabUnder.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity49(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard11.appendChild(newDiv);
        }
    });
    quantity.innerText = count;
}

function changeQuantity49(key, quantity) {
    if (quantity == 0) {
        delete mainTabUnder[key];
    } else {
        mainTabUnder[key].quantity = quantity;
        mainTabUnder[key].price = quantity * products[key].price;
    }
    reloadCard()
}
