let mainGamingAccessories1 = document.querySelector('.main-GamingAccessories1');
let mainGamingAccessories2 = document.querySelector('.main-GamingAccessories2');
...
let listCard27 = document.querySelector('.listCard27');
...
let mainGamAccessories1 = [];
let mainGamAccessories2 = [];

function initApp2() {
    productGamingAccessories1.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamAccessories(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingAccessories1.appendChild(newDiv)
    });
    productGamingAccessories2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box" >
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamAccessories2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingAccessories2.appendChild(newDiv)
    });

}
initApp2()
function addToCardGamAccessories(key){
    if(mainGamAccessories1[key] == null) {
        mainGamAccessories1[key] = productGamingAccessories1[key];
        mainGamAccessories1[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardGamAccessories2(key){
    if(mainGamAccessories2[key] == null) {
        mainGamAccessories2[key] = productGamingAccessories2[key];
        mainGamAccessories2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard27.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainGamAccessories1.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity9(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard27.appendChild(newDiv);
        }
    });
    mainGamAccessories2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity10(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard27.appendChild(newDiv);
        }
...
function changeQuantity9(key, quantity) {
    if (quantity == 0) {
        delete mainGamAccessories1[key];
    } else {
        mainGamAccessories1[key].quantity = quantity;
        mainGamAccessories1[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity10(key, quantity) {
    if (quantity == 0) {
        delete mainGamAccessories2[key];
    } else {
        mainGamAccessories2[key].quantity = quantity;
        mainGamAccessories2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
