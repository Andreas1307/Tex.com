let mainTablets32gb = document.querySelector('.main-tablet32gb');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard15 = document.querySelector('.listCard15');
...
function initApp2() {
    productTablets32gb.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTab32gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTablets32gb.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTab32gb(key){
    if(mainTab32gb[key] == null) {
        mainTab32gb[key] = productTablets32gb[key];
        mainTab32gb[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard15.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainTab32gb.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity45(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard15.appendChild(newDiv);
        }
    });
...
function changeQuantity45(key, quantity) {
    if (quantity == 0) {
        delete mainTab32gb[key];
    } else {
        mainTab32gb[key].quantity = quantity;
        mainTab32gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
