let mainGamingOver1 = document.querySelector('.main-GamingOver');
...
let listCard25 = document.querySelector('.listCard25');
...
function initApp2() {
    productGamingOver1.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamOver(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingOver1.appendChild(newDiv)
    });

}
initApp2()
function addToCardGamOver(key){
    if(mainGamOver1[key] == null) {
        mainGamOver1[key] = productGamingOver1[key];
        mainGamOver1[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard25.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainGamOver1.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity13(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard25.appendChild(newDiv);
        }
    });
...
function changeQuantity13(key, quantity) {
    if (quantity == 0) {
        delete mainGamOver1[key];
    } else {
        mainGamOver1[key].quantity = quantity;
        mainGamOver1[key].price = quantity * products[key].price;
    }
    reloadCard()
}
