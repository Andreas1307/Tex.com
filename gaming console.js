let mainGamingConsole1 = document.querySelector('.main-GamingConsole1');
let mainGamingConsole2 = document.querySelector('.main-GamingConsole2');
...
let listCard26 = document.querySelector('.listCard26');
...
let productGamingConsole2 = [
    ...
    {
        id: 2,
        name: 'Console SONY PlayStation 4 Slim (PS4 Slim) 500GB, Jet Black, F-Chassis',
        image: 'ps4 1.jpg',
        price: '$300.99',
        source: 'ps4 pp.html'
    }
...
function initApp2() {
    productGamingConsole1.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamConsole(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingConsole1.appendChild(newDiv)
    });
    productGamingConsole2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardGamConsole2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainGamingConsole2.appendChild(newDiv)
    });

}
initApp2()
function addToCardGamConsole(key){
    if(mainGamConsole1[key] == null) {
        mainGamConsole1[key] = productGamingConsole1[key];
        mainGamConsole1[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardGamConsole2(key){
    if(mainGamConsole2[key] == null) {
        mainGamConsole2[key] = productGamingConsole2[key];
        mainGamConsole2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard26.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainGamConsole1.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity11(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard26.appendChild(newDiv);
        }
    });
    mainGamConsole2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity12(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard26.appendChild(newDiv);
        }
...
function changeQuantity11(key, quantity) {
    if (quantity == 0) {
        delete mainGamConsole1[key];
    } else {
        mainGamConsole1[key].quantity = quantity;
        mainGamConsole1[key].price = quantity * products[key].price;
    }
    reloadCard()
}

function changeQuantity12(key, quantity) {
    if (quantity == 0) {
        delete mainGamConsole2[key];
    } else {
        mainGamConsole2[key].quantity = quantity;
        mainGamConsole2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
