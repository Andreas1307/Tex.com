let mainTablets64gb = document.querySelector('.main-tablet64gb');
let mainTablets641gb = document.querySelector('.main-tablet641gb');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard14 = document.querySelector('.listCard14');
...
function initApp2() {
    productTablets64gb.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTab64gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTablets64gb.appendChild(newDiv)
    });

    productTablets641gb.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    <button class="heartBtn" onclick="alert('You need an account first')"><i class="fa-regular fa-heart"></i></button>
                    <img class="valueImg" src="img/${value.image}">
                    <div class="slider-text">
                        <p class="value-name">${value.name}</p>
                        <p class="in-stock"><i class="fa-solid fa-barcode"></i> in stock</p>
                        <div class="stars">
                        ...
                        <button onclick="addToCardTab641gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTablets641gb.appendChild(newDiv)
    });
}
initApp2()
function addToCardTab64gb(key){
    if(mainTab64gb[key] == null) {
        mainTab64gb[key] = productTablets64gb[key];
        mainTab64gb[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardTab641gb(key){
    if(mainTab641gb[key] == null) {
        mainTab641gb[key] = productTablets641gb[key];
        mainTab641gb[key].quantity = 1;
 }    
 reloadCard();
}

function reloadCard() {
    listCard14.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainTab64gb.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity46(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard14.appendChild(newDiv);
        }
    });

    mainTab641gb.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity47(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard14.appendChild(newDiv);
        }
...
function changeQuantity46(key, quantity) {
    if (quantity == 0) {
        delete mainTab64gb[key];
    } else {
        mainTab64gb[key].quantity = quantity;
        mainTab64gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity47(key, quantity) {
    if (quantity == 0) {
        delete mainTab641gb[key];
    } else {
        mainTab641gb[key].quantity = quantity;
        mainTab641gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
