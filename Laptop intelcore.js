let mainLaptopIntelcore = document.querySelector('.main-laptopIntelcore');
let mainLaptopIntelcore2 = document.querySelector('.main-laptopIntelcore2');
...
let listCard20 = document.querySelector('.listCard20');
...
let productLaptopsIntelcore2 = [
    {
        id: 5,
        name: 'Laptop Gaming ASUS TUF F15 FX507ZC4-HN073, Intel Core i5, 16GB, SSD 512GB, NVIDIA GeForce RTX 3050 4GB',
...
let mainIntelcore2 = [];
function initApp2() {
    productLaptopsIntelcore.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapIntelcore(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopIntelcore.appendChild(newDiv)
    });

    productLaptopsIntelcore2.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapIntelcore2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopIntelcore2.appendChild(newDiv)
    });
}
initApp2()
function addToCardLapIntelcore(key){
    if(mainIntelcore[key] == null) {
        mainIntelcore[key] = productLaptopsIntelcore[key];
        mainIntelcore[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardLapIntelcore2(key){
    if(mainIntelcore2[key] == null) {
        mainIntelcore2[key] = productLaptopsIntelcore2[key];
        mainIntelcore2[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard20.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainIntelcore.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity21(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard20.appendChild(newDiv);
        }
    });

    mainIntelcore2.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity22(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard20.appendChild(newDiv);
        }
...
function changeQuantity21(key, quantity) {
    if (quantity == 0) {
        delete mainIntelcore[key];
    } else {
        mainIntelcore[key].quantity = quantity;
        mainIntelcore[key].price = quantity * products[key].price;
    }
    reloadCard()
}

function changeQuantity22(key, quantity) {
    if (quantity == 0) {
        delete mainIntelcore2[key];
    } else {
        mainIntelcore2[key].quantity = quantity;
        mainIntelcore2[key].price = quantity * products[key].price;
    }
    reloadCard()
}
