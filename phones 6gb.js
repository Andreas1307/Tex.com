let main6gb = document.querySelector('.main-6gb');
let main61gb = document.querySelector('.main-61gb');

let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard9 = document.querySelector('.listCard9');
...
function initApp2() {
    productContent6gb.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCard6gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        main6gb.appendChild(newDiv)
    });

    productContent61gb.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCard61gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        main61gb.appendChild(newDiv)
    });
}
initApp2()
function addToCard6gb(key){
    if(mainList6gb[key] == null) {
        mainList6gb[key] = productContent6gb[key];
        mainList6gb[key].quantity = 1;
 }    
 reloadCard();
}
function addToCard61gb(key){
    if(mainList61gb[key] == null) {
        mainList61gb[key] = productContent61gb[key];
        mainList61gb[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    mainList6gb.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity31(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard9.appendChild(newDiv);
        }
    });
    mainList61gb.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity32(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard9.appendChild(newDiv);
        }
...
function changeQuantity31(key, quantity) {
    if (quantity == 0) {
        delete mainList6gb[key];
    } else {
        mainList6gb[key].quantity = quantity;
        mainList6gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity32(key, quantity) {
    if (quantity == 0) {
        delete mainList61gb[key];
    } else {
        mainList61gb[key].quantity = quantity;
        mainList61gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
