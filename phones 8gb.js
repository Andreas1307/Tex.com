let main8gb = document.querySelector('.main-8gb');
let main81gb = document.querySelector('.main-81gb');

let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard8 = document.querySelector('.listCard8');
...
function initApp2() {
    productContent8gb.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCard8gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        main8gb.appendChild(newDiv)
    });

    productContent81gb.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCard81gb(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        main81gb.appendChild(newDiv)
    });
}
initApp2()
function addToCard8gb(key){
    if(mainList8gb[key] == null) {
        mainList8gb[key] = productContent8gb[key];
        mainList8gb[key].quantity = 1;
 }    
 reloadCard();
}
function addToCard81gb(key){
    if(mainList81gb[key] == null) {
        mainList81gb[key] = productContent81gb[key];
        mainList81gb[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    mainList8gb.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity33(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard8.appendChild(newDiv);
        }
    });

    mainList81gb.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity34(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard8.appendChild(newDiv);
        }
...
function changeQuantity33(key, quantity) {
    if (quantity == 0) {
        delete mainList8gb[key];
    } else {
        mainList8gb[key].quantity = quantity;
        mainList8gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
function changeQuantity34(key, quantity) {
    if (quantity == 0) {
        delete mainList81gb[key];
    } else {
        mainList81gb[key].quantity = quantity;
        mainList81gb[key].price = quantity * products[key].price;
    }
    reloadCard()
}
