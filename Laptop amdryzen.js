let mainLaptopAmdryzen = document.querySelector('.main-laptopAmdryzen');
...
let listCard21 = document.querySelector('.listCard21');
...
let mainAmdryzen2 = [];
function initApp2() {
    productLaptopsAmdryzen.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardLapAmdryzen(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainLaptopAmdryzen.appendChild(newDiv)
    });
}
initApp2()
function addToCardLapAmdryzen(key){
    if(mainAmdryzen[key] == null) {
        mainAmdryzen[key] = productLaptopsAmdryzen[key];
        mainAmdryzen[key].quantity = 1;
 }    
 reloadCard();
}

function reloadCard() {
    listCard21.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainAmdryzen.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity20(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard21.appendChild(newDiv);
        }
    });
    quantity.innerText = count;
}

function changeQuantity20(key, quantity) {
    if (quantity == 0) {
        delete mainAmdryzen[key];
    } else {
        mainAmdryzen[key].quantity = quantity;
        mainAmdryzen[key].price = quantity * products[key].price;
    }
    reloadCard()
}
