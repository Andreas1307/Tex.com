let mainTvOver = document.querySelector('.main-tvOver');



let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard30 = document.querySelector('.listCard30');
...
function initApp2() {
    productTvOver.forEach((value, key) => {
        ...
        <div class="slider-box">
                    ...
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardTvOver(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainTvOver.appendChild(newDiv)
    });

   
}
initApp2()
function addToCardTvOver(key){
    if(mainListTvOver[key] == null) {
        mainListTvOver[key] = productTvOver[key];
        mainListTvOver[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    listCard30.innerHTML = '';
    let count = 0;
    let totalPrice = 0;

    mainListTvOver.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity55(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard30.appendChild(newDiv);
        }
    });
...
function changeQuantity55(key, quantity) {
    if (quantity == 0) {
        delete mainListTvOver[key];
    } else {
        mainListTvOver[key].quantity = quantity;
        mainListTvOver[key].price = quantity * products[key].price;
    }
    reloadCard()
}
