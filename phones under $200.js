let mainProductsUnder21 = document.querySelector('.main-under21');
let mainProductsUnder22 = document.querySelector('.main-under22');


let openShopping = document.querySelector('.cart');
let closeShopping = document.querySelector('.closeShopping');
let listCard3 = document.querySelector('.listCard3');
...
let mainUnder2002 = [];
function initApp2() {
    productContentUnder21.forEach((value, key) => {
        ...
        <div class="slider-box">
                   ...
                   <a href="${value.source}"><img class="valueImg" src="img/${value.image}"></a>
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardUnder(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainProductsUnder21.appendChild(newDiv)
    });
    productContentUnder22.forEach((value, key) => {
        let newDiv = document.createElement('div');
        newDiv.classList.add('item')
        newDiv.innerHTML = `
        <div class="slider-box">
                   ...
                   <a href="${value.source}"><img class="valueImg" src="img/${value.image}"></a>
                    <div class="slider-text">
                        ...
                        <button onclick="addToCardUnder2(${key})">Add To Cart</button>
                    </div>
                </div>
        `
        mainProductsUnder22.appendChild(newDiv)
    });

}
initApp2()
function addToCardUnder(key){
    if(mainUnder200[key] == null) {
        mainUnder200[key] = productContentUnder21[key];
        mainUnder200[key].quantity = 1;
 }    
 reloadCard();
}
function addToCardUnder2(key){
    if(mainUnder2002[key] == null) {
        mainUnder2002[key] = productContentUnder22[key];
        mainUnder2002[key].quantity = 1;
 }    
 reloadCard();
}
...
function reloadCard() {
    ...
    let count = 0;
    let totalPrice = 0;
    mainUnder200.forEach((value, key) => {
        ...
        if(value != null) {
            ...
            <div>
            <button class="changeQuantity" onclick="changeQuantity37(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard3.appendChild(newDiv);
        }
    });

    mainUnder2002.forEach((value, key) => {
        totalPrice = totalPrice + value.price;
        count = count + value.quantity;
        ...
        if(value != null) {
            ...
            <div>${value.price.toLocaleString()}</div>  
            <div>
            <button class="changeQuantity" onclick="changeQuantity38(${key}, ${value.quantity - 1})">Remove</button>
            </div>
            `
            listCard3.appendChild(newDiv);
        }
...
function changeQuantity37(key, quantity) {
    if (quantity == 0) {
        delete mainUnder200[key];
    } else {
        mainUnder200[key].quantity = quantity;
        mainUnder200[key].price = quantity * products[key].price;
    }
    reloadCard()
}

function changeQuantity38(key, quantity) {
    if (quantity == 0) {
        delete mainUnder2002[key];
    } else {
        mainUnder2002[key].quantity = quantity;
        mainUnder2002[key].price = quantity * products[key].price;
    }
    reloadCard()
}
